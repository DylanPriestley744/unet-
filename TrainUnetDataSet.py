...
def Train_Unet(net, device, data_path, batch_size=3, epochs=40, lr=0.0001):
    ...
    print(f"加载到的图片路径: {train_dataset}")  # 调试代码
    print(f"加载到的图片数量: {len(train_dataset)}")  # 调试代码
    ...
    for epoch in range(epochs):
        ...
        running_loss = 0.0
        i = 0
        begin = time.perf_counter()  # 修改为 time.perf_counter
        for image, label in train_loader:
            ...
            opt.step()
        end = time.perf_counter()  # 修改为 time.perf_counter
        ...
        if loss_avg_epoch < bes_los:
            ...
            torch.save(state, 'model.pth')  # 修改保存文件名为 model.pth
...
if __name__ == '__main__':
    ...
    # 对应数据集的路径
    data_path = r"d:\unet\Unet-main\train_image"  # 修改为绝对路径
    Train_Unet(net, device, data_path, epochs=5, batch_size=1)
